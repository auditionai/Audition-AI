...
-- TST keeps the existing capacity (4 system / 1 per user for each media type).
-- Gommo owns an independent lane with twice that processing capacity.

create index if not exists idx_generated_images_provider_queue_capacity
  on public.generated_images (provider, status, asset_type, created_at)
  where coalesce(queue_kind, '') in ('image_generate', 'video_generate', 'motion_generate');
...
declare
    ...
    v_system_image_limit integer := 4;
    v_system_video_limit integer := 4;
    v_user_image_limit integer := 1;
    v_user_video_limit integer := 1;
...
begin
    ...
    if v_provider = 'gommo' then
        v_system_image_limit := 8;
        v_system_video_limit := 8;
        v_user_image_limit := 2;
        v_user_video_limit := 2;
    end if;
    ...
    into v_my_image_processing, v_my_video_processing, v_my_queued,
         v_system_image_processing, v_system_video_processing, v_system_queued
    from public.generated_images gi
    where gi.status in ('queued', 'processing')
      and coalesce(gi.queue_kind, '') in ('image_generate', 'video_generate', 'motion_generate')
      and (case when lower(coalesce(nullif(gi.provider, ''), gi.queue_payload->>'__targetProvider', 'tst')) = 'gommo' then 'gommo' else 'tst' end) = v_provider;

    if v_asset_type = 'image' then
        v_can_dispatch_now := v_my_image_processing < v_user_image_limit
          and v_system_image_processing < v_system_image_limit;
    else
        v_can_dispatch_now := v_my_video_processing < v_user_video_limit
          and v_system_video_processing < v_system_video_limit;
    end if;

    -- Queue limits remain unchanged, but are now isolated per provider.
    if not v_can_dispatch_now and v_my_queued >= 1 then raise exception 'USER_QUEUE_LIMIT_REACHED'; end if;
    if not v_can_dispatch_now and v_system_queued >= 10 then raise exception 'SYSTEM_QUEUE_FULL'; end if;

    if v_cost > 0 then
        v_charge_applied := public.apply_balance_transaction(
            p_user_id, -v_cost, coalesce(p_tool_name, p_queue_kind, 'Generated Job'),
            'usage', 'generated_image_charge', p_id::text,
            jsonb_build_object(
                'generated_image_id', p_id, 'tool_id', p_tool_id,
                'queue_kind', p_queue_kind, 'asset_type', v_asset_type,
                'provider', v_provider, 'cost_vcoin', v_cost
            )
        );
...
as $$
  with normalized as (
    select gi.*,
      case when lower(coalesce(nullif(gi.provider, ''), gi.queue_payload->>'__targetProvider', 'tst')) = 'gommo'
        then 'gommo' else 'tst' end as provider_key
    from public.generated_images gi
  ...
  eligible as (
    select
      n.id, n.user_id, n.asset_type, n.created_at, n.provider_key,
      case when n.provider_key = 'gommo' then 8 else 4 end
        - coalesce(p.active_count, 0) as system_slots,
      case when n.provider_key = 'gommo' then 2 else 1 end
        - coalesce(up.active_count, 0) as user_slots
    from normalized n
    left join processing p
      on p.provider_key = n.provider_key and p.media_type = coalesce(n.asset_type, 'image')
    left join user_processing up
      on up.user_id = n.user_id and up.provider_key = n.provider_key
    ...
    where n.status = 'queued'
      ...
      and (case when n.provider_key = 'gommo' then 8 else 4 end) - coalesce(p.active_count, 0) > 0
      and (case when n.provider_key = 'gommo' then 2 else 1 end) - coalesce(up.active_count, 0) > 0
  ),
  ranked_user as (
    select e.*,
      row_number() over (
  ...
  ranked_system as (
    select ru.*,
      row_number() over (
        partition by ru.provider_key, coalesce(ru.asset_type, 'image')
  ...
  picked as (
    ...
    where rs.rn_system <= rs.system_slots
...
as $$
  with scoped as (
    select user_id, status, coalesce(asset_type, 'image') as media_type,
      case when lower(coalesce(nullif(provider, ''), queue_payload->>'__targetProvider', 'tst')) = 'gommo'
        then 'gommo' else 'tst' end as provider_key
