...
const LuckyWheelModal: React.FC<LuckyWheelModalProps> = ({ isOpen, onClose }) => {
    ...
    const handleSpin = async () => {
        ...
        try {
            ...
            // 3. Calculate the center angle of the target segment
            // Since we construct segments starting from 0deg (top-right due to skew), 
            // let's align calculation with rendering logic.
            // Current pointer is at Top (0deg).
            // To bring target segment to Top, we rotate the wheel such that target segment moves to 0.
            // Segment i spans from (i * segmentAngle) to ((i+1) * segmentAngle).
            // Center of segment i is at (i + 0.5) * segmentAngle.
            // To bring this center to 0 (Top), we rotate by -(i + 0.5) * segmentAngle.
            // Adding 360 * spins for effect.
            
            // BUT: The CSS implementation rotates the wheel container.
            // A positive rotation moves the wheel Clockwise.
            // So segment 0 starts at 0 and moves clockwise.
            // To get segment i to the top (0deg), we need to rotate Counter-Clockwise by its position?
            // Or Rotate Clockwise by (360 - position)?
            
            // Let's use a simpler heuristic:
            // Target Angle = (targetIndex * segmentAngle) + (segmentAngle / 2);
            // Final Rotation = CurrentRotation + (360 * 5) + (360 - TargetAngle);
            // Add some randomness within the segment (+/- 40% of half segment)
            const randomOffset = (Math.random() - 0.5) * (segmentAngle * 0.8);
            
            const targetAngle = (targetIndex * segmentAngle) + (segmentAngle / 2);
            const spins = 5; 
            
            // We need to ensure we always rotate forward
            const nextRotation = rotation + (360 * spins) + (360 - targetAngle % 360) + randomOffset;
            
            // Correction: If (360 - targetAngle) is smaller than current modulus, we might spin backwards?
            // No, we add 360*spins.
            // Let's just ensure delta is positive and large.
                    ...
                    >
                        {rewards.map((reward, index) => {
                            ...
                            return (
                                ...
                                >
                                    {/* Content Container - Unskew and Rotate to align with bisector */}
                                    {/* 
                                      The logic here puts the content div's bottom-left corner at the wheel center.
                                      We unskew by skewY(skewAngle) and rotate by half the segment angle to align x-axis with bisector.
                                    */}
                                    <div 
                                    ...
                                    >
                                        {/* 
                                          Now we are in a standard coordinate system relative to the slice center line.
                                          We want to position text along the "radius" which is now roughly the X-axis (bottom edge of this div).
                                          Let's move it out from center.
                                        */}
                                        <div 
                                            className="absolute bottom-0 left-1/2 w-full h-16 origin-bottom-left flex items-center justify-start pl-12 sm:pl-16 gap-2"
                                            style={{
                                                transform: 'rotate(-90deg) translate(-50%, 50%)', // Rotate -90 to align perpendicular to radius (reading outward)? No, along radius.
                                                // Actually, easier method:
                                                // Current Axis: Bottom edge (y=0) is bisector. (0,0) is center.
                                                // We want text to run from center OUTWARDS along X-axis.
                                                // So we position at bottom: 0, left: some offset.
                                                // And rotate text if needed? No, standard text runs left-to-right.
                                                // If we want text to read "outwards" (head to center), we might need rotation.
                                                // Let's try standard placement along X-axis.
                                                
                                                bottom: '-20px', // Center vertically on the bisector line (height is 40px, so -20px)
                                                left: '40px', // Distance from center
                                                height: '40px',
                                                width: '140px', // Max width
                                                // display: 'flex', alignItems: 'center', justifyContent: 'flex-start' is default div
                                            }}
                                        >
