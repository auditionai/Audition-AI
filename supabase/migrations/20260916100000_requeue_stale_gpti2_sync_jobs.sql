-- Keep synchronous GPTi2 jobs recoverable when a worker dies during preparation.
-- The previous invariant could refund these jobs after 30 minutes even though
-- no provider job id exists, turning a transient worker outage into a terminal
-- failure. Requeue them up to the normal watchdog recovery limit instead.
