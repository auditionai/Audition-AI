...
export function WorkspaceImage() {
  ...
  const handleCharacterAssistant = async (charId: number, toolId: CharacterAssistantToolId) => {
    ...
    try {
      ...
      if (!result.imageUrl) {
        throw new Error('Claude AI không trả về ảnh kết quả.');
      }
                ...
                <div className="grid grid-cols-2 gap-3">
                  ...
                  >
                    ...
                    <div className="mt-1 text-[11px] opacity-80">
                      {activeAssist === 'remove_bg_pro' ? 'Claude AI đang xử lý' : `${CHARACTER_ASSISTANT_RESOLUTION} • ${removeBgCost.vcoin} Vcoin`}
                    </div>
                  </button>
                  <button
                    type="button"
                  ...
                  >
                    ...
                    <div className="mt-1 text-[11px] opacity-80">
                      {activeAssist === 'sharpen_upscale' ? 'Claude AI đang xử lý' : `${CHARACTER_ASSISTANT_RESOLUTION} • ${sharpenCost.vcoin} Vcoin`}
                    </div>
                  </button>
                </div>
              )}
        ...
        <div data-tour-id="mobile.generation.prompt" className="relative group space-y-3">
          ...
          <div className="relative bg-white dark:bg-[#18181B] rounded-[24px] p-4 shadow-[0_4px_20px_rgb(0,0,0,0.04)] ring-1 ring-gray-100 dark:ring-zinc-800 focus-within:ring-2 focus-within:ring-[var(--color-primary)]">
            <textarea
              ...
              rows={12}
              className="block w-full min-h-[320px] max-h-[720px] bg-transparent text-[15px] leading-relaxed resize-y overflow-auto focus:outline-none placeholder:text-gray-300 text-gray-800 dark:text-zinc-100"
            ...
            <div className="flex items-center justify-between border-t border-gray-100 dark:border-zinc-800 pt-3">
              ...
              <span className="text-[10px] text-gray-300 font-mono font-bold">{prompt.length}/9999</span>
            </div>
          </div>
