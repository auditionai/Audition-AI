...
export const CLAUDE_DEFAULT_TIMEOUT_MS = Number(process.env.CLAUDE_REQUEST_TIMEOUT_MS || 120_000);
...
export const CLAUDE_BACKGROUND_TIMEOUT_MS = parseBoundedTimeout(process.env.CLAUDE_BACKGROUND_TIMEOUT_MS, 300_000);
...
export const isClaudeApiKey = (value: unknown) => {
  const key = String(value || '').trim();
  return key.length >= 8 && !/\s/.test(key) && !key.startsWith('{');
};

export const createClaudeClient = (apiKey: string) => new OpenAI({
  apiKey,
  baseURL: OPENAI_COMPATIBLE_BASE_URL,
  timeout: Number.isFinite(CLAUDE_DEFAULT_TIMEOUT_MS) && CLAUDE_DEFAULT_TIMEOUT_MS > 0 ? CLAUDE_DEFAULT_TIMEOUT_MS : 120_000,
  maxRetries: 0,
});
...
const toVisionDataUrl = async (image: ClaudeImageInput) => {
...
export const getClaudeApiKey = async () => {
  ...
  if (environmentKey) {
    if (!isClaudeApiKey(environmentKey)) throw new Error('CLAUDE_NOT_CONFIGURED: The Claude API key is invalid.');
  ...
  const row = (data || []).find((candidate) => isClaudeApiKey(candidate.key_value));
