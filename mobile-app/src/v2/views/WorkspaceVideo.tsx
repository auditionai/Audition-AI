...
export function WorkspaceVideo() {
    ...
    const loadCatalog = async (forceRefresh = false) => {
      try {
        ...
        const liveVideoModels = getVideoModelSpecs(livePricing, filteredModels)
          .filter((spec: any) => isModelAllowedForFeature(routingConfig, 'video_generation', spec.modelId))
          .filter((spec: any) => resolveProviderForModel(routingConfig, spec.modelId, 'video_generation') === 'tst')
          .map((spec: any) => ({
          id: spec.modelId,
        ...
        const liveMotionModels = getMotionModelSpecs(livePricing, filteredModels)
          ...
          .filter((spec: any) => resolveProviderForModel(routingConfig, spec.modelId, 'motion_control') === 'tst')
          .map((spec: any) => ({
          id: spec.modelId,
          name: spec.displayName,
  ...
  const gommoMotionSeconds = Math.max(1, Math.ceil(motionVideoDurationSeconds || 1));
  const selectedGommoPricing = isGommoMotionSelected ? gommoMotionPricing : gommoPricing;
  const currentCostBreakdown = isGommoSelected
    ? {
        ...
        vcoin: isGommoMotionSelected ? (selectedGommoPricing?.vcoin || 0) * gommoMotionSeconds : (selectedGommoPricing?.vcoin || 0),
        billingUnit: (isGommoMotionSelected ? 'second' : 'job') as 'second' | 'job',
        unitVcoin: isGommoMotionSelected ? (selectedGommoPricing?.vcoin || 0) : null,
        billedSeconds: isGommoMotionSelected ? gommoMotionSeconds : null,
      }
    : tstCostBreakdown;
  ...
  const applyVideoModelConfig = (modelId: string) => {
    if (resolveProviderForModel(providerConfig, modelId, 'video_generation') === 'gommo') {
      const model = getGommoModelForAudition(gommoCatalog, modelId);
      if (model?.resolutions[0]) setQuality(model.resolutions[0].type.toUpperCase());
      if (model?.durations[0]) setDuration(`${model.durations[0].type}S`.toUpperCase());
      if (model?.ratios[0]) setAspectRatio(model.ratios[0].type);
      if (model?.modes[0]) setProviderMode(model.modes[0].type);
      setSound(false);
      return;
    }
  ...
  const handleGenerate = async () => {
    ...
    void (async () => {
      try {
        ...
        const effectiveServerId = isGommoSelected ? undefined : activeMode === 'video_ai'
        ...
        const effectiveSpeedId = isGommoSelected ? undefined : activeMode === 'video_ai'
        ...
        const queuePayload = activeMode === 'video_ai'
          ...
          : {
              ...
              backgroundSource: isGommoMotionSelected ? 'input_image' : undefined,
        ...
        {activeMode === 'video_ai' ? (
          <div className="space-y-6">
            ...
            <div data-tour-id="mobile.video.settings" className="grid grid-cols-2 gap-4">
              ...
              {activeMode === 'video_ai' && modelOptions.durations.length > 0 && <div className="space-y-2">
              ...
              {modelOptions.qualities.length > 0 && <div className="space-y-2">
              ...
              {speedOptions.length > 0 && <div className="space-y-2">
              ...
              {serverOptions.length > 0 && <div className="space-y-2">
            ...
            {/* Audio Toggle */}
            {!isGommoVideoSelected && modelOptions.supportsAudio && (
