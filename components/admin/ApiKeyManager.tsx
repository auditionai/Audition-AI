...
const ApiKeyManager: React.FC = () => {
    ...
    const fetchApiKeys = useCallback(async () => {
        ...
        try {
            ...
            if (!res.ok) throw new Error('Không thể tải khóa API.');
    ...
    const handleCreate = async (e: React.FormEvent) => {
        ...
        try {
            ...
            showToast('Thêm khóa API thành công!', 'success');
        } catch (e: any) {
            showToast(e.message, 'error');
        } finally {
            setIsCreating(false);
    ...
    const handleDelete = async (id: string) => {
        if (!window.confirm('Bạn chắc chắn muốn xóa khóa API này?')) return;
        try {
    ...
    return (
        <div className="bg-[#12121A]/80 border border-red-500/20 rounded-2xl shadow-lg p-6">
            <h3 className="text-2xl font-bold mb-4 text-red-400">Quản Lý Khóa API</h3>
            <form onSubmit={handleCreate} className="grid grid-cols-12 gap-2 p-4 bg-black/20 rounded-lg mb-6">
                 <input type="text" placeholder="Tên gợi nhớ" value={newName} onChange={e => setNewName(e.target.value)} className="auth-input col-span-4" required />
