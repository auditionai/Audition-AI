...
const callClaude = async (path: string, body: Record<string, unknown>) => {
  ...
  if (!response.ok) throw new Error(String(data?.error || `Claude request failed (${response.status})`));
  return data;
};
...
export const analyzeStyleImage = async (imageBase64: string): Promise<string> => {
  const data = await callClaude('grok-style-analysis', { image: imageBase64 });
  return String(data?.analysis || '').trim();
};

export const checkConnection = async (key?: string): Promise<ConnectionResult> => {
  try {
  const data = await callClaude('grok-health', key ? { key } : {});
