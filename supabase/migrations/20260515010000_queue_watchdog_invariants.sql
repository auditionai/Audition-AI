...
begin
  with candidates as (
    select
      ...
      coalesce(gi.processing_started_at, gi.created_at, v_now) as started_at
      ...
      queue_payload =
        ...
        || jsonb_build_object(
          ...
          '__logs', coalesce(gi.queue_payload -> '__logs', '[]'::jsonb) || jsonb_build_array(jsonb_build_object(
            ...
            'stage', 'queued',
            'level', 'warning',
            'message', 'DB invariant: job processing chua co provider id bi het lease. Dua lai hang doi tu dong.'
          ))
        ),
      lease_token = null,
