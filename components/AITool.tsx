...
const AITool: React.FC = () => {
    // Fix: Use `useAuth` to get context values.
    ...
    const handleBgRemovalImageUpload = (e: React.ChangeEvent<HTMLInputElement>) => {
        ...
        // where the type was being inferred as `unknown`, causing an error on `readAsDataURL`.
    ...
    const handleMoveToGenerator = (image: {processedUrl: string, file: File}) => {
        ...
        setTimeout(() => {
            document.getElementById('character-uploader')?.scrollIntoView({ behavior: 'smooth', block: 'center' });
        }, 100);
    };
            ...
            <div className="grid grid-cols-1 lg:grid-cols-10 gap-6">
                ...
                <div className="lg:col-span-3">
                    <div className="bg-[#1a1a22]/80 rounded-2xl border border-white/10 shadow-lg p-4 md:p-6 h-full flex flex-col">
                        <div className="text-center mb-6">
                            <h2 className="text-2xl font-bold flex items-center justify-center gap-3"><i className="ph-fill ph-person-simple-run text-pink-400"></i>Tách Nền & Làm Nét</h2>
                            <p className="text-gray-400 mt-1 text-sm max-w-md mx-auto">Tải ảnh lên, AI sẽ tự động làm nét và loại bỏ phông nền.</p>
                        </div>
                            ...
                            <div className="flex flex-col flex-grow">
                                ...
                                <div className="bg-black/20 rounded-lg border border-white/10 flex-grow p-4 min-h-[14rem]">
                                    {processedImages.length === 0 && !isProcessing ? (
                                        ...
                                        <div className="grid grid-cols-2 sm:grid-cols-3 gap-4 h-full overflow-y-auto custom-scrollbar">
                ...
                <div className="lg:col-span-7">
                     <div className="bg-[#1a1a22]/80 rounded-2xl border border-white/10 shadow-lg p-4 md:p-6">
                        <div className="text-center mb-6">
                            <h2 className="text-2xl font-bold flex items-center justify-center gap-3"><i className="ph-fill ph-magic-wand text-pink-400"></i>Công Cụ Tạo Ảnh AI</h2>
                        </div>
                        <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
                            ...
                            {/* Result */}
                            <div className="lg:col-span-1 lg:sticky top-24 h-[calc(100vh-7rem)] bg-black/20 p-4 rounded-2xl border border-white/5 flex flex-col items-center justify-between">
                                 <div className="w-full flex-grow aspect-square bg-black/30 rounded-lg flex items-center justify-center overflow-hidden my-2 border border-white/5 relative">
                                    {isLoading ? ( <GenerationProgress currentStep={generationStep} /> ) : 
                                     ...
                                     ( <div className="text-center text-gray-500"><i className="ph-fill ph-image-square text-5xl"></i><p className="mt-2 text-sm">Hình ảnh của bạn sẽ xuất hiện ở đây</p><p className="text-xs">Hãy điền câu lệnh và nhấn "Tạo ảnh" để bắt đầu.</p></div> )}
