...
-- Avoid materializing the full generated_images row (especially the large
-- queue_payload TOAST value) before projecting the compact gallery response.
-- Gallery history is intentionally limited to the recent 30-day window.
...
as $$
  with recent_refs as materialized (
    select
  ...
  rows as materialized (
    select
      gi.id,
      gi.image_url,
      left(coalesce(gi.prompt, ''), 4000) as prompt,
      gi.created_at,
      gi.updated_at,
      gi.asset_type,
      gi.queue_kind,
...
revoke execute on function public.get_user_gallery_images_lightweight(uuid, integer)
  from public, anon, authenticated;
grant execute on function public.get_user_gallery_images_lightweight(uuid, integer)
  to service_role;
