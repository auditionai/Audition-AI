-- GPTi2 image edits are synchronous. GPT Image 2.5 can render for several
-- minutes, so the watchdog must not refund a live, lease-protected request.
  ...
  for v_refund_id in
    with candidates as (
      ...
      where gi.status = 'processing' and gi.job_id is null
        ...
        -- GPTi2 synchronous dispatch is intentionally protected for ten
        -- minutes. The queue worker holds a matching long lease during this.
        and not (lower(coalesce(gi.queue_payload ->> '__targetProvider', gi.provider, '')) = 'gpti2'
          ...
          and gi.updated_at > v_now - interval '10 minutes')
      limit 100
    ), updated as (
