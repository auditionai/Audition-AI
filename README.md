...
Gommo integration uses the current Jobs gateway (`https://v2.api.gommo.net` by default): Bearer authentication, live `GET /ai/models?type=...` discovery, `POST /ai/jobs/{type}/{model_id}` submission, and `POST /ai/jobs/{id}?media=...` polling. Set `GOMMO_API_BASE` only when Gommo supplies a different staging or white-label gateway. TST model options and pricing are likewise read from live `/v1/models` and `/v1/models/pricing`; the application accepts both the current `pricing[].key` field and the temporary legacy `config_key` alias.
...
Provider routing can also be overridden per function: single image, couple, every group size from 3-8, prompt image, video, and motion control. Groups 6-8 default to Gommo; group 8 uses exactly eight character references and never sends a sample image. The Admin pricing table merges live TST and Gommo configurations and includes a `Chưa có giá` filter for exact configurations without an AUDITION AI Vcoin price.

Each function route also has an independent allowed-model list in **Bảng giá**. `Mặc định` uses the application policy, `Tất cả` removes the restriction, and selecting individual live models exposes only those models on both desktop and mobile. Groups 6-8 default to `image-gpt-2` only. The queue API enforces the same list before pricing and enqueue, so an outdated or modified client cannot submit a disabled model. This setting is stored in `generation_provider_mode.allowedModelsByFeature`; no database migration is required.
