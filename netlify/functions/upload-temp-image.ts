...
const handler: Handler = async (event: HandlerEvent) => {
    ...
    try {
        ...
        // Store in a temp folder. Lifecycle rules on R2 should auto-delete these after 24h ideally.
        const ext = mimeType.split('/')[1] || 'png';
        const fileName = `temp/${user.id}/${Date.now()}_${Math.floor(Math.random() * 10000)}.${ext}`;
        ...
        console.error("Upload temp image failed:", error);
