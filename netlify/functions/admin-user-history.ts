...
const ADMIN_LEDGER_FETCH_LIMIT = 600;
...
const mapPaymentTransactionToHistoryItem = (tx: any): HistoryItem => ({
  ...
  discountAmount: toNumber(tx.discount_amount_vnd ?? tx.provider_payload?.discount_amount_vnd),
  originalAmount: toNumber(tx.original_amount_vnd ?? tx.provider_payload?.original_amount_vnd ?? tx.amount_vnd),
  category: 'topup',
...
const mapVcoinTransactionToHistoryItem = (
  ...
  return {
    ...
    topupGiftcode: relatedPayment?.topup_giftcode || relatedPayment?.provider_payload?.topup_giftcode || null,
    discountAmount: relatedPayment ? toNumber(relatedPayment.discount_amount_vnd ?? relatedPayment.provider_payload?.discount_amount_vnd) : undefined,
    originalAmount: relatedPayment ? toNumber(relatedPayment.original_amount_vnd ?? relatedPayment.provider_payload?.original_amount_vnd ?? relatedPayment.amount_vnd) : undefined,
    category,
