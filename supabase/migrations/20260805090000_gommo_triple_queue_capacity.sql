...
begin
    ...
    if v_provider = 'gommo' then
        v_system_image_limit := 12;
        v_system_video_limit := 12;
        v_user_image_limit := 3;
        v_user_video_limit := 3;
    end if;
...
as $$
  ...
  eligible as (
    select
      ...
      case when n.provider_key = 'gommo' then 12 else 4 end
        - coalesce(p.active_count, 0) as system_slots,
      case when n.provider_key = 'gommo' then 3 else 1 end
    ...
    where n.status = 'queued'
      ...
      and (case when n.provider_key = 'gommo' then 12 else 4 end) - coalesce(p.active_count, 0) > 0
      and (case when n.provider_key = 'gommo' then 3 else 1 end) - coalesce(up.active_count, 0) > 0
  ),
  ranked_user as (
    select e.*,
      row_number() over (
