...
const UserManager: React.FC = () => {
    ...
    if (isLoading) return <p className="text-center text-gray-400 p-8">Đang tải danh sách người dùng...</p>;

    return (
        <div className="bg-[#12121A]/80 border border-purple-500/20 rounded-2xl shadow-lg p-6">
            <h3 className="text-2xl font-bold mb-4 text-purple-400">Quản Lý Người Dùng</h3>
            <div className="max-h-96 overflow-y-auto custom-scrollbar pr-2">
                <div className="grid grid-cols-12 gap-4 text-xs font-bold text-gray-400 uppercase p-2 border-b border-white/10">
                    ...
                    <div className="col-span-2">XP / Cấp</div>
                ...
                {users.map(user => (
                    <div key={user.id} className="grid grid-cols-12 gap-4 items-center p-2 border-b border-white/5 text-sm">
                        ...
                        <div className="col-span-2 text-right">
                            <button onClick={() => handleEditClick(user)} className="px-3 py-1 bg-blue-500/20 text-blue-300 text-xs rounded-md">Chỉnh sửa</button>
                        </div>
                    </div>
                ))}
            </div>
            ...
            {isModalOpen && editingUser && (
                <Modal isOpen={isModalOpen} onClose={() => setIsModalOpen(false)} title={`Chỉnh sửa: ${editingUser.display_name}`}>
                    <div className="space-y-4">
                        ...
                        <div className="flex items-center">
                            ...
                            />
                            <label htmlFor="is_admin" className="ml-2 block text-sm text-gray-300">Là Admin?</label>
                        </div>
                        <div className="flex justify-end gap-3 pt-4">
                            <button onClick={() => setIsModalOpen(false)} className="themed-button-secondary">Hủy</button>
                            <button onClick={handleSave} disabled={isSaving} className="themed-button-primary">{isSaving ? 'Đang lưu...' : 'Lưu thay đổi'}</button>
                        </div>
                    </div>
                </Modal>
            )}
