...
export const Landing: React.FC<LandingProps> = ({ onEnter }) => {
  ...
  // 1. Load Showcase Images (Community Gallery)
  useEffect(() => {
    const loadShowcase = async () => {
        // Default Mock Data (Fallback content when DB is empty)
        const mockItems = [
            { 
                title: "CYBERPUNK", 
                author: "@NeoArtist", 
                img: "https://picsum.photos/400/600?random=10", 
                border: "border-audi-pink" 
            },
            { 
                title: "FANTASY", 
                author: "@DragonSlayer", 
                img: "https://picsum.photos/400/600?random=11", 
                border: "border-audi-purple" 
            },
            { 
                title: "VINTAGE", 
                author: "@ClassicVibe", 
                img: "https://picsum.photos/400/600?random=12", 
                border: "border-audi-cyan" 
            },
            { 
                title: "CHIBI", 
                author: "@KawaiiMode", 
                img: "https://picsum.photos/400/600?random=13", 
                border: "border-audi-lime" 
            },
            { 
                title: "MECHA", 
                author: "@BotMaker", 
                img: "https://picsum.photos/400/600?random=14", 
                border: "border-audi-pink" 
            },
            { 
                title: "FASHION", 
                author: "@StyleQueen", 
                img: "https://picsum.photos/400/600?random=15", 
                border: "border-audi-yellow" 
            },
        ];
        ...
        try {
            ...
            if (realImages && realImages.length > 0) {
                ...
                const formattedRealImages = realImages.map((img: GeneratedImage, idx: number) => {
                    // Cycle through border colors for visual variety
                    const borders = [
                        "border-audi-pink", 
                        "border-audi-purple", 
                        "border-audi-cyan", 
                ...
                // Merge real images first, then mock data to keep the marquee full
  ...
  // 2. Rhythm Simulator (Beat Effect) - Syncs with typical 130 BPM song
  useEffect(() => {
  ...
  const handleAuthAction = async () => {
      ...
      try {
          if (supabase) {
              ...
              // Mock Login if Supabase is offline/not configured
              console.log("Offline Mode Login");
              onEnter();
          }
      } catch (e: any) {
  ...
  return (
    ...
    >
      ...
      {/* -------------------------------------------------- */}
      {/* 1. BACKGROUND LAYER (Effects & 3D Floor)           */}
      {/* -------------------------------------------------- */}
      <div className="fixed inset-0 pointer-events-none z-0 overflow-hidden">
          ...
          <div 
            className={`absolute top-0 left-1/4 w-0.5 md:w-1 h-[150vh] bg-audi-pink blur-md origin-top animate-[spin_4s_ease-in-out_infinite] opacity-30`} 
            style={{ transform: `rotate(${mousePos.x}deg)` }}
          ></div>
          <div 
            className={`absolute top-0 right-1/4 w-0.5 md:w-1 h-[150vh] bg-audi-cyan blur-md origin-top animate-[spin_5s_ease-in-out_infinite_reverse] opacity-30`} 
            style={{ transform: `rotate(${-mousePos.x}deg)` }}
          ></div>
      </div>

      {/* -------------------------------------------------- */}
      {/* 2. PROMOTION TICKER (Sticky Top)                   */}
      {/* -------------------------------------------------- */}
      ...
      {/* -------------------------------------------------- */}
      {/* 5. AI SHOWCASE (Horizontal Marquee)                */}
      {/* -------------------------------------------------- */}
      ...
      {/* -------------------------------------------------- */}
      {/* 6. FEATURES SECTION (Detailed Grid)                */}
      {/* -------------------------------------------------- */}
      ...
      {showLogin && createPortal(
          <div className="fixed inset-0 z-[100] flex items-center justify-center bg-black/80 backdrop-blur-sm p-4 animate-fade-in">
              <div className="w-full max-w-md bg-[#090014] border-2 border-audi-pink rounded-3xl p-8 relative shadow-[0_0_50px_rgba(255,0,153,0.3)]">
                  ...
                  {/* --- GOOGLE QUICK LOGIN BUTTON (Added) --- */}
                  <div className="mb-6">
                      <button 
