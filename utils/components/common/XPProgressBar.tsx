...
const XPProgressBar: React.FC<XPProgressBarProps> = ({ currentXp, currentLevel }) => {
    ...
    return (
        <div>
            <h4 className="font-semibold text-white mb-2 text-sm">Tiến độ cấp bậc</h4>
            ...
            <div className="flex justify-between text-xs text-gray-400 mt-1.5">
                ...
                {nextRank && xpRemaining > 0 && (
                    <span>
                        Cần {xpRemaining.toLocaleString()} XP nữa để đạt <span className="font-bold text-pink-300">{nextRank.title}</span>
                    </span>
                )}
                 {xpRemaining === 0 && (
                     <span className="font-bold text-green-400">Đã đạt cấp tối đa!</span>
                 )}
            </div>
        </div>
    );
