...
const handler: Handler = async (event: HandlerEvent) => {
    ...
    try {
        ...
        const payload = JSON.parse(jobData.prompt); // The JSON payload we stored in spawner
        ...
        // Overwrite the complex payload JSON in 'prompt' column with the simple user prompt string for clean display
        await Promise.all([
