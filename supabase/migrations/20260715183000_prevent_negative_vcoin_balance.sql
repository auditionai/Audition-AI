...
-- Balance checks must happen while the user row is locked. Client-side profile
-- data can be stale, and concurrent generation requests must not overdraw the
-- same balance between a separate SELECT and UPDATE.
...
revoke execute on function public.apply_balance_transaction(uuid, numeric, text, text, text, text, jsonb)
  from public, anon, authenticated;
grant execute on function public.apply_balance_transaction(uuid, numeric, text, text, text, text, jsonb)
  to service_role;
