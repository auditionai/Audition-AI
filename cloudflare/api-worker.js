...
const allowedOrigins = new Set(['https://auditionai.io.vn', 'https://develop--audition-ai.netlify.app']);
const withCors = (response, request) => { const headers = new Headers(response.headers); const origin = request?.headers.get('origin') || ''; if (allowedOrigins.has(origin)) { headers.set('access-control-allow-origin', origin); headers.set('vary', 'Origin'); } headers.set('access-control-allow-headers', 'authorization, content-type, x-audition-device-key, x-secret-key, x-cron-secret'); headers.set('access-control-allow-methods', 'GET, POST, PUT, OPTIONS'); return new Response(response.body, { status: response.status, headers }); };
...
const fromBase64Url = (value) => atob(String(value || '').replace(/-/g, '+').replace(/_/g, '/').padEnd(Math.ceil(String(value || '').length / 4) * 4, '='));
const signUploadToken = async (env, payload) => { const secret = text(env, 'UPLOAD_TOKEN_SECRET'); if (!secret) throw new Error('UPLOAD_TOKEN_SECRET is not configured'); const encoded = base64Url(JSON.stringify(payload)); const key = await crypto.subtle.importKey('raw', new TextEncoder().encode(secret), { name: 'HMAC', hash: 'SHA-256' }, false, ['sign']); const signature = new Uint8Array(await crypto.subtle.sign('HMAC', key, new TextEncoder().encode(encoded))); return `${encoded}.${base64Url(String.fromCharCode(...signature))}`; };
const verifyUploadToken = async (env, token) => { const [encoded, signature] = String(token || '').split('.'); if (!encoded || !signature) throw Object.assign(new Error('Invalid upload token'), { status: 401 }); const expected = await signUploadToken(env, JSON.parse(fromBase64Url(encoded))); const valid = await timingSafeEqual(expected, `${encoded}.${signature}`); if (!valid) throw Object.assign(new Error('Invalid upload token'), { status: 401 }); const payload = JSON.parse(fromBase64Url(encoded)); if (!payload?.id || !payload?.userId || !payload?.key || !payload?.contentType || !Number.isFinite(payload?.exp) || payload.exp <= Date.now()) throw Object.assign(new Error('Upload token expired'), { status: 401 }); return payload; };
...
const repairAndAssertCompleteUploadedImage = (source, type) => { let bytes = source; const has = (...values) => values.every((value, index) => bytes[index] === value); const endsWith = (...values) => values.every((value, index) => bytes[bytes.length - values.length + index] === value); if (type === 'image/jpeg' && bytes.length > 2 && has(0xff, 0xd8) && !endsWith(0xff, 0xd9)) { const repaired = new Uint8Array(bytes.length + 2); repaired.set(bytes); repaired[bytes.length] = 0xff; repaired[bytes.length + 1] = 0xd9; bytes = repaired; } let valid = bytes.length > 0; if (type === 'image/jpeg') valid = valid && has(0xff, 0xd8) && endsWith(0xff, 0xd9); if (type === 'image/png') valid = valid && has(0x89, 0x50, 0x4e, 0x47) && endsWith(0x49, 0x45, 0x4e, 0x44, 0xae, 0x42, 0x60, 0x82); if (type === 'image/gif') valid = valid && (has(0x47, 0x49, 0x46, 0x38, 0x37, 0x61) || has(0x47, 0x49, 0x46, 0x38, 0x39, 0x61)) && endsWith(0x3b); if (type === 'image/webp') valid = valid && has(0x52, 0x49, 0x46, 0x46) && bytes[8] === 0x57 && bytes[9] === 0x45 && bytes[10] === 0x42 && bytes[11] === 0x50; if (!valid) throw Object.assign(new Error('Uploaded image is truncated or corrupt. Please re-upload the original file.'), { status: 422 }); return bytes; };
...
const notifyOperationalAlert = async (env, title, details, severity = 'warning') => { const webhook = text(env, 'TELEGRAM_NOTIFY_WEBHOOK_URL'); const secret = text(env, 'TELEGRAM_NOTIFY_WEBHOOK_SECRET'); if (!webhook || !secret) return; const response = await fetch(webhook, { method: 'POST', headers: { 'content-type': 'application/json', 'x-notify-secret': secret }, body: JSON.stringify({ eventType: 'queue_alert', app: 'Audition AI', alert: { title, details, severity, key: 'sepay_reconcile_cloudflare', createdAt: new Date().toISOString() } }), signal: AbortSignal.timeout(8000) }); if (!response.ok) throw new Error(`Telegram alert failed (${response.status})`); };
...
const reconcileSePay = async (env, limit = 10) => {
  ...
  const pendingOlderThanFiveMinutes = transactions.filter((tx) => String(tx.status || '').toLowerCase() === 'pending' && Date.now() - new Date(tx.created_at || 0).getTime() > 5 * 60 * 1000).length;
  const failed = results.filter((item) => item.status >= 400).length;
  if (pendingOlderThanFiveMinutes || failed) await notifyOperationalAlert(env, 'SePay reconcile can chu y', { checked: transactions.length, settled: results.filter((item) => item.settled).length, pendingOlderThanFiveMinutes, failed, results: results.filter((item) => !item.settled).slice(0, 10) }, pendingOlderThanFiveMinutes ? 'error' : 'warning');
  return { checked: transactions.length, settled: results.filter((item) => item.settled).length, pendingOlderThanFiveMinutes, failed, results };
};
...
async function handle(request, env, ctx) {
  ...
  if (path === 'storage-upload-url') { const user = await requireUser(request, env); if (request.method === 'GET') return json({ ready: true, provider: 'cloudflare-r2' }); if (request.method !== 'POST') return json({ error: 'Method Not Allowed' }, 405); const body = await request.json(); const type = String(body.contentType || '').toLowerCase().split(';', 1)[0]; if (!/^(image|video)\/[a-z0-9.+-]+$/i.test(type)) return json({ error: 'Unsupported file type' }, 400); const key = `users/${user.id}/${cleanSegment(body.folder) || 'inputs'}/${Date.now()}-${crypto.randomUUID()}.${extensionFor(type)}`; const id = crypto.randomUUID(); const exp = Date.now() + 300000; const created = await db(env, 'cloudflare_upload_tokens', { method: 'POST', headers: { Prefer: 'return=minimal' }, body: JSON.stringify({ id, user_id: user.id, object_key: key, content_type: type, expires_at: new Date(exp).toISOString() }) }); if (!created.ok) throw new Error(`Unable to create upload token (${created.status})`); const token = await signUploadToken(env, { id, userId: user.id, key, contentType: type, exp }); return json({ provider: 'cloudflare-r2', uploadUrl: `${url.origin}/api/storage-upload?token=${encodeURIComponent(token)}`, publicUrl: `${text(env, 'R2_PUBLIC_URL').replace(/\/+$/, '')}/${key}`, expiresIn: 300 }); }
  if (path === 'storage-upload' && request.method === 'PUT') { const token = await verifyUploadToken(env, url.searchParams.get('token')); const key = String(token.key); const type = String(request.headers.get('content-type') || '').toLowerCase().split(';', 1)[0]; if (type !== token.contentType || !key.startsWith(`users/${token.userId}/`) || !/^users\/[\w-]+\/[\w./-]+\.(png|jpg|jpeg|webp|gif|mp4|mov|webm|bin)$/i.test(key)) return json({ error: 'Invalid upload token scope' }, 403); const length = Number(request.headers.get('content-length') || 0); if (length > 50 * 1024 * 1024) return json({ error: 'File too large' }, 413); let bytes = new Uint8Array(await request.arrayBuffer()); if (bytes.length > 50 * 1024 * 1024) return json({ error: 'File too large' }, 413); if (type.startsWith('image/')) bytes = repairAndAssertCompleteUploadedImage(bytes, type); const consumed = await rpc(env, 'consume_cloudflare_upload_token', { p_id: token.id, p_user_id: token.userId, p_object_key: key, p_content_type: type }); if (consumed !== true) return json({ error: 'Upload token expired or already used' }, 401); await env.RESULTS_BUCKET.put(key, bytes, { httpMetadata: { contentType: type } }); return new Response(null, { status: 204 }); }
